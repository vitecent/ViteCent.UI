<template>
	<div class="dictionary-container layout-padding">
		<div class="dictionary-padding layout-padding-view layout-padding-auto">
			<Table v-bind="state.tableData" class="dictionary" @pageChange="onTablePageChange" @sortHeader="onSortHeader">
...
const state = reactive<EmptyObjectType>({
	tableData: {
		...
		header: [
			{ key: 'type', title: t('message.dictionary.type'), type: 'text', isCheck: true },
			{ key: 'code', title: t('message.dictionary.code'), type: 'text', isCheck: true },
			{ key: 'name', title: t('message.dictionary.name'), type: 'text', isCheck: true },
			{ key: 'abbreviation', title: t('message.dictionary.abbreviation'), type: 'text', isCheck: true },
			{ key: 'server', title: t('message.dictionary.server'), type: 'text', isCheck: true },
			{ key: 'port', title: t('message.dictionary.port'), type: 'text', isCheck: true },
			{ key: 'user', title: t('message.dictionary.user'), type: 'text', isCheck: true },
			{ key: 'password', title: t('message.dictionary.password'), type: 'text', isCheck: true },
			{ key: 'charSet', title: t('message.dictionary.charSet'), type: 'text', isCheck: true },
		],
		data: [] as Dictionary[],
		// 打印标题
		printName: t('message.router.dataDictionary'),
	},
	form: {
		name: '',
		email: '',
...
// 新增
const onAdd = () => {
	router.push({ name: 'addDictionary' });
};
// 修改
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editDictionary', params: { id: row.scope.id } });
};
// 删除
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'dictionaryDetails', params: { id: row.scope.id } });
};
// 分页改变时回调
