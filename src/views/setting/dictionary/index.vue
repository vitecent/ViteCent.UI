<template>
	<section class="dictionary-container layout-padding">
...
const state = reactive<EmptyObjectType>({
	data: {
		...
		data: [] as Dictionary[],
		printName: t('message.router.settingDictionary'),
	},
	form: {
		name: '',
	},
...
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editDictionary', query: { id: row.scope.id } });
};
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'dictionaryDetails', query: { id: row.scope.id } });
};
