<template>
	<div class="position-container layout-padding">
		<div class="position-padding layout-padding-view layout-padding-auto">
			<Table v-bind="state.tableData" class="position" @pageChange="onTablePageChange" @sortHeader="onSortHeader">
...
const state = reactive<EmptyObjectType>({
	tableData: {
		...
		header: [
			{ key: 'type', title: t('message.position.type'), type: 'text', isCheck: true },
			{ key: 'code', title: t('message.position.code'), type: 'text', isCheck: true },
			{ key: 'name', title: t('message.position.name'), type: 'text', isCheck: true },
			{ key: 'abbreviation', title: t('message.position.abbreviation'), type: 'text', isCheck: true },
			{ key: 'server', title: t('message.position.server'), type: 'text', isCheck: true },
			{ key: 'port', title: t('message.position.port'), type: 'text', isCheck: true },
			{ key: 'user', title: t('message.position.user'), type: 'text', isCheck: true },
			{ key: 'password', title: t('message.position.password'), type: 'text', isCheck: true },
			{ key: 'charSet', title: t('message.position.charSet'), type: 'text', isCheck: true },
		],
		data: [] as Position[],
		// 打印标题
		printName: t('message.router.dataPosition'),
	},
	form: {
		name: '',
		email: '',
...
// 新增
const onAdd = () => {
	router.push({ name: 'addPosition' });
};
// 修改
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editPosition', params: { id: row.scope.id } });
};
// 删除
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'positionDetails', params: { id: row.scope.id } });
};
// 分页改变时回调
