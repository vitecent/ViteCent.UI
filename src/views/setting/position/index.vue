<template>
	<section class="position-container layout-padding">
...
const state = reactive<EmptyObjectType>({
	data: {
		...
		data: [] as Position[],
		printName: t('message.router.settingPosition'),
	},
	form: {
		name: '',
	},
...
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editPosition', query: { id: row.scope.id } });
};
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'positionDetails', query: { id: row.scope.id } });
};
