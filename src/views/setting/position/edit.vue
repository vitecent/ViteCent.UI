<template>
	<section class="position-container">
		<h3 class="mb30">{{ $t('message.router.editPosition') }}</h3>
			...
			<el-row :gutter="10">
				<el-col :md="12" class="mb15">
					<el-form-item :label="$t('message.position.type')" prop="type">
						<el-input v-model="state.form.type" :placeholder="$t('message.position.typePlaceholder')" maxlength="50" show-word-limit clearable />
					</el-form-item>
				</el-col>
				<el-col :md="12" class="mb15">
		...
		if (valid) {
			api
				...
				.then((res) => {
					...
					state.form = {} as Position;

					if (state.flag) router.push({ name: 'settingPosition' });
					else initData();
				})
				.catch((error) => {});
		} else {
