<template>
	<section class="user-container layout-padding">
...
const state = reactive<EmptyObjectType>({
	data: {
		...
		header: [
			{ key: 'type', title: t('message.users.type'), type: 'text', isCheck: true },
			{ key: 'code', title: t('message.users.code'), type: 'text', isCheck: true },
			{ key: 'name', title: t('message.users.name'), type: 'text', isCheck: true },
			{ key: 'abbreviation', title: t('message.users.abbreviation'), type: 'text', isCheck: true },
			{ key: 'server', title: t('message.users.server'), type: 'text', isCheck: true },
			{ key: 'port', title: t('message.users.port'), type: 'text', isCheck: true },
			{ key: 'user', title: t('message.users.user'), type: 'text', isCheck: true },
			{ key: 'password', title: t('message.users.password'), type: 'text', isCheck: true },
			{ key: 'charSet', title: t('message.users.charSet'), type: 'text', isCheck: true },
		],
		data: [] as User[],
		printName: t('message.router.settingUser'),
	},
	form: {
		name: '',
	},
...
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editUser', query: { id: row.scope.id } });
};
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'userDetails', query: { id: row.scope.id } });
};
