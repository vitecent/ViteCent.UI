<template>
	<div class="user-container layout-padding">
		<div class="user-padding layout-padding-view layout-padding-auto">
			<Table v-bind="state.tableData" class="user" @pageChange="onTablePageChange" @sortHeader="onSortHeader">
...
const state = reactive<EmptyObjectType>({
	tableData: {
		...
		header: [
			{ key: 'type', title: t('message.user.type'), type: 'text', isCheck: true },
			{ key: 'code', title: t('message.user.code'), type: 'text', isCheck: true },
			{ key: 'name', title: t('message.user.name'), type: 'text', isCheck: true },
			{ key: 'abbreviation', title: t('message.user.abbreviation'), type: 'text', isCheck: true },
			{ key: 'server', title: t('message.user.server'), type: 'text', isCheck: true },
			{ key: 'port', title: t('message.user.port'), type: 'text', isCheck: true },
			{ key: 'user', title: t('message.user.user'), type: 'text', isCheck: true },
			{ key: 'password', title: t('message.user.password'), type: 'text', isCheck: true },
			{ key: 'charSet', title: t('message.user.charSet'), type: 'text', isCheck: true },
		],
		data: [] as User[],
		// 打印标题
		printName: t('message.router.dataUser'),
	},
	form: {
		name: '',
		email: '',
...
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editUser', params: { id: row.scope.id } });
};
// 删除
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'userDetails', params: { id: row.scope.id } });
};
// 分页改变时回调
