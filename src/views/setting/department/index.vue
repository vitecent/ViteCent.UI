<template>
	<div class="department-container layout-padding">
		<div class="department-padding layout-padding-view layout-padding-auto">
			<Table v-bind="state.tableData" class="department" @pageChange="onTablePageChange" @sortHeader="onSortHeader">
...
const state = reactive<EmptyObjectType>({
	tableData: {
		...
		header: [
			{ key: 'type', title: t('message.department.type'), type: 'text', isCheck: true },
			{ key: 'code', title: t('message.department.code'), type: 'text', isCheck: true },
			{ key: 'name', title: t('message.department.name'), type: 'text', isCheck: true },
			{ key: 'abbreviation', title: t('message.department.abbreviation'), type: 'text', isCheck: true },
			{ key: 'server', title: t('message.department.server'), type: 'text', isCheck: true },
			{ key: 'port', title: t('message.department.port'), type: 'text', isCheck: true },
			{ key: 'user', title: t('message.department.user'), type: 'text', isCheck: true },
			{ key: 'password', title: t('message.department.password'), type: 'text', isCheck: true },
			{ key: 'charSet', title: t('message.department.charSet'), type: 'text', isCheck: true },
		],
		data: [] as Department[],
		// 打印标题
		printName: t('message.router.dataDepartment'),
	},
	form: {
		name: '',
		email: '',
...
// 新增
const onAdd = () => {
	router.push({ name: 'addDepartment' });
};
// 修改
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editDepartment', params: { id: row.scope.id } });
};
// 删除
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'departmentDetails', params: { id: row.scope.id } });
};
// 分页改变时回调
