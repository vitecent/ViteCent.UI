<template>
	<div class="company-container layout-padding">
		<div class="company-padding layout-padding-view layout-padding-auto">
			<Table v-bind="state.tableData" class="company" @pageChange="onTablePageChange" @sortHeader="onSortHeader">
...
const state = reactive<EmptyObjectType>({
	tableData: {
		...
		header: [
			{ key: 'type', title: t('message.company.type'), type: 'text', isCheck: true },
			{ key: 'code', title: t('message.company.code'), type: 'text', isCheck: true },
			{ key: 'name', title: t('message.company.name'), type: 'text', isCheck: true },
			{ key: 'abbreviation', title: t('message.company.abbreviation'), type: 'text', isCheck: true },
			{ key: 'server', title: t('message.company.server'), type: 'text', isCheck: true },
			{ key: 'port', title: t('message.company.port'), type: 'text', isCheck: true },
			{ key: 'user', title: t('message.company.user'), type: 'text', isCheck: true },
			{ key: 'password', title: t('message.company.password'), type: 'text', isCheck: true },
			{ key: 'charSet', title: t('message.company.charSet'), type: 'text', isCheck: true },
		],
		data: [] as Company[],
		// 打印标题
		printName: t('message.router.dataCompany'),
	},
	form: {
		name: '',
		email: '',
...
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editCompany', params: { id: row.scope.id } });
};
// 删除
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'companyDetails', params: { id: row.scope.id } });
};
// 分页改变时回调
