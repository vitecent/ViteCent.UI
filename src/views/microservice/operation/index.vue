<template>
	<div class="operation-container layout-padding">
		<div class="operation-padding layout-padding-view layout-padding-auto">
			<Table v-bind="state.tableData" class="operation" @pageChange="onTablePageChange" @sortHeader="onSortHeader">
...
const state = reactive<EmptyObjectType>({
	tableData: {
		...
		header: [
			{ key: 'type', title: t('message.operation.type'), type: 'text', isCheck: true },
			{ key: 'code', title: t('message.operation.code'), type: 'text', isCheck: true },
			{ key: 'name', title: t('message.operation.name'), type: 'text', isCheck: true },
			{ key: 'abbreviation', title: t('message.operation.abbreviation'), type: 'text', isCheck: true },
			{ key: 'server', title: t('message.operation.server'), type: 'text', isCheck: true },
			{ key: 'port', title: t('message.operation.port'), type: 'text', isCheck: true },
			{ key: 'user', title: t('message.operation.user'), type: 'text', isCheck: true },
			{ key: 'password', title: t('message.operation.password'), type: 'text', isCheck: true },
			{ key: 'charSet', title: t('message.operation.charSet'), type: 'text', isCheck: true },
		],
		data: [] as Operation[],
		// 打印标题
		printName: t('message.router.dataOperation'),
	},
	form: {
		name: '',
		email: '',
...
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editOperation', params: { id: row.scope.id } });
};
// 删除
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'operationDetails', params: { id: row.scope.id } });
};
// 分页改变时回调
