<template>
	<section class="operation-container layout-padding">
...
const state = reactive<EmptyObjectType>({
	data: {
		...
		data: [] as Operation[],
		printName: t('message.router.microserviceOperation'),
	},
	form: {
		name: '',
	},
...
const onEdit = (row: EmptyObjectType) => {
	router.push({ name: 'editOperation', query: { id: row.scope.id } });
};
...
const onDetails = (row: EmptyObjectType) => {
	router.push({ name: 'operationDetails', query: { id: row.scope.id } });
};
