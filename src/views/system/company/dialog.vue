					...
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="公司类型">
							<el-radio-group v-model="state.ruleForm.menuType">
								<el-radio label="menu">公司</el-radio>
					...
					<template v-if="state.ruleForm.menuType === 'menu'">
						...
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
							<el-form-item label="公司图标">
								<IconSelector placeholder="请输入公司图标" v-model="state.ruleForm.meta.icon" />
							</el-form-item>
						</el-col>
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
...
const state = reactive({
	// 参数请参考 `/src/router/route.ts` 中的 `dynamicRoutes` 路由公司格式
	ruleForm: {
		menuSuperior: [], // 上级公司
		menuType: 'menu', // 公司类型
		name: '', // 路由名称
		...
		btnPower: '', // 公司类型为按钮时，权限标识
	},
	menuData: [] as RouteItems, // 上级公司数据
	dialog: {
		isShowDialog: false,
		type: '',
		title: '',
...
const openDialog = (type: string, row?: any) => {
	if (type === 'edit') {
		...
		state.dialog.title = '修改公司';
		state.dialog.submitTxt = '修 改';
	} else {
		state.dialog.title = '新增公司';
...
const onSubmit = () => {
	...
	// setBackEndControlRefreshRoutes() // 刷新公司，未进行后端接口测试
};
// 页面加载时
onMounted(() => {
